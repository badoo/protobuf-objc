...
namespace google { namespace protobuf { namespace compiler {namespace objectivec {
  ...
  FileGenerator::~FileGenerator() {
    ...
    void FileGenerator::GenerateAggregateHeader(io::Printer* printer, string enumsHeaderName) {
        ...
        if (!enumsHeaderName.empty()) {
            printer->Print("#import \"$value$\"\n\n", "value", enumsHeaderName);
        }
        ...
        string systemPrefix = "PB";
        string optionsClass = "ObjectiveCFileOptions";
        string stringToSearch = "@class ";
        string buildStringToSearch = "_Builder";
        for (set<string>::const_iterator i(dependencies.begin()); i != dependencies.end(); ++i) {
            string str = *i;
            bool isNotABuilderClass = str.find(buildStringToSearch) == string::npos;
            if (isNotABuilderClass) {
                string className = str.substr(stringToSearch.length());
                bool isOwnClass = className.compare(0, systemPrefix.length(), systemPrefix) != 0;
                bool notAnOptionsClass = className != optionsClass;
                if (isOwnClass && notAnOptionsClass)
                    printer->Print("#import \"$value$.pb.h\"\n", "value", className);
            }
        }

        printer->Print("\n\n");
        printer->Print("#ifndef __has_feature\n"
        ...
        printer->Print("+ (PBExtensionRegistry*) extensionRegistry;\n"
    ...
    void FileGenerator::GenerateEnumsHeader(io::Printer* printer) {
        printer->Print("// Generated by the protocol buffer compiler.  DO NOT EDIT!\n\n");
    ...
    void FileGenerator::GenerateHeaders(OutputDirectory* outputDirectory, string extension, string aggregateHeaderName) {
        
        for (int i = 0; i < file_->message_type_count(); i++) {
            const Descriptor* descriptor = file_->message_type(i);
            string generatedClassName = ClassName(descriptor);
            
            scoped_ptr<io::ZeroCopyOutputStream> output(outputDirectory->Open(generatedClassName + ".pb.h"));
            io::Printer printer(output.get(), '$');
            
            printer.Print("// Generated by the protocol buffer compiler.  DO NOT EDIT!\n\n");
            
            if (!aggregateHeaderName.empty()) {
                printer.Print("#import \"$value$\"\n\n", "value", aggregateHeaderName);
            }
  ...
  void FileGenerator::GenerateAggregateSource(io::Printer* printer) {
    FileGenerator file_generator(file_);
    string header_file = FileName(file_) + ".pb.h";
  ...
  void FileGenerator::GenerateSeparateSources(OutputDirectory* outputDirectory, string extension, string aggregateHeaderName) {

    for (int i = 0; i < file_->message_type_count(); i++) {
      const Descriptor* descriptor = file_->message_type(i);
      string generatedClassName = ClassName(descriptor);

      scoped_ptr<io::ZeroCopyOutputStream> output(outputDirectory->Open(generatedClassName + extension));
      io::Printer printer(output.get(), '$');
      ...
      //this is there to supperess the warning regarding 
      printer.Print("#pragma clang diagnostic push\n");
      printer.Print("#pragma clang diagnostic ignored \"-Wshadow-ivar\"\n\n");
